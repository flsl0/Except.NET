...
internal class Lib
{
    ...
    public static double Divide(double a, double b) => Check<DivisorCantBeZero>(b != 0)
        .Do(() => a / b);
...
public class Api
{
    ...
    public double Divide(double dividend, double divisor) => Try(() => Lib.Divide(dividend, divisor))
        .Catch<DivisorCantBeZero>(double.PositiveInfinity);

    public string GetVersion() => Try(Lib.GetVersion)
        .Catch(_ => "0.0.0-Dev");

    public List<string> GetLabelsByIds(int[] arr) => ForEach(arr, Lib.GetLabelById)
        .CatchAll(HandleExceptions);

    public string GetLabelById(int labelId) => Try(() => Lib.GetLabelById(labelId))
        .Catch<LabelNotFound>(Throw)
        .Catch<DefaultLabelNotFound>(Throw);

    public static Api CreateApi() => (Api) Try(_CreateApi)
